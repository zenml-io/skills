...
**A note on "real-time"**: When users say "real-time inference," clarify what they actually mean. ZenML *does* have a deployment concept — `pipeline.deploy()` creates an HTTP-serving endpoint that wraps a pipeline. This works well for many real-time use cases (chatbot backends, document processing APIs, recommendation requests, fraud detection, LLM agent workflows). ZenML deployers also support performance tuning options — for example, **disabling materialization** within the deployed pipeline can dramatically reduce per-request latency by skipping artifact serialization/deserialization overhead. Each deployer has specific tips in the [deployment docs](https://docs.zenml.io/how-to/deployment/deployment) for achieving good latency.
